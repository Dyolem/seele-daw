...
| Global Bar          | 品牌入口、项目名、保存状态、全局菜单   | 顶部全局区               | 含两种 MIDI 导入入口    |
...
- Workbench 项目菜单 MUST 分别提供“导入为新项目”和“导入为当前项目的新 Track”；Arrangement
  最后一个 Track Lane 下方与空态只提供后者，不能用一个含糊入口隐藏目标 Project 语义。
- “导入为新项目”必须先只读并完整验证 MIDI，再根据最新当前项目状态复用 Save / Discard /
...
20. Workbench MIDI 导入保留“新项目”和“当前项目新 Track”两种显式意图；Arrangement 入口只追加
    新 Track。当前项目模式保留既有 Tempo / 拍号、一次提交整个文件并由 Undo 一次撤销。
