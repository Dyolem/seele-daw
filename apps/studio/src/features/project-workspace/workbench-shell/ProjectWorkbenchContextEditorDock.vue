...
const selectedBuiltInInstrumentId = computed(
  () =>
    (selectedInstrument.value?.status === PROJECT_TRACK_INSTRUMENT_STATUS.READY
      ? selectedInstrument.value.soundbankId
      : null) ?? '',
)
const instrumentSelectorPlaceholder = computed(() =>
  selectedInstrument.value?.status === PROJECT_TRACK_INSTRUMENT_STATUS.READY
    ? 'Choose an instrument'
    : 'Choose a replacement',
)
const instrumentSelectorText = computed(() =>
  selectedBuiltInInstrumentId.value.length > 0 && selectedInstrument.value !== null
    ? selectedInstrument.value.displayName
    : instrumentSelectorPlaceholder.value,
)
...
function selectBuiltInInstrument(soundbankIdInput: unknown): void {
  const track = props.selectedTrack
  if (track === null || typeof soundbankIdInput !== 'string' || soundbankIdInput.length === 0) {
    return
  }

  try {
    projectTracks.selectBuiltInInstrument(track.id, parseSoundbankId(soundbankIdInput))
  } catch (cause) {
    toasts.danger(
      ...
      >
        ...
        >
          ...
          <template v-else>
            ...
            <code
              v-if="
                selectedInstrument.soundbankId !== null || selectedInstrument.deviceTypeId !== null
              "
            >
              {{ selectedInstrument.soundbankId ?? selectedInstrument.deviceTypeId }}
            </code>
          </template>
...
.project-workbench__instrument-selector {
  display: grid;
  gap: var(--sd-space-1);
  margin-block-start: var(--sd-space-1);
}

.project-workbench__instrument-selector > span {
  color: var(--sd-color-text-muted);
  font-size: var(--sd-font-size-xs);
  font-weight: 650;
}
