...
| 来源证据封套  | MIDI Source Envelope        | MI6B 用于携带文件/容器、消息协议和语义证据状态的中立契约；这里的 Envelope 不是控制声音起落的 ADSR 包络。MI6C 又加入限定 Mode Declaration 检查结果。  |
...
- **“尚未检查声明”不等于“没有声明”**：普通构造路径仍可能保留 `unresolved`；MI6C 限定检查也只
  识别五类 Mode Declaration。无论哪种状态，都不能据此断言文件是 GM、非 GM 或没有演奏法控制。
