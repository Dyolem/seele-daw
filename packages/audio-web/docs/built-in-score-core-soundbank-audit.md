...
本文件只证明本地资产可以被确定性规范化，不表示 Studio 已经提供多乐器选择、Program 导入路由
或产品分发。本批生成的 Manifest/WAV 和库存 JSON 全部位于被 Git 忽略、被 production build
排除的 `apps/studio/public/soundbanks`。
