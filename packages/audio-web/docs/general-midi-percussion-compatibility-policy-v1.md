...
> Status: MI1C implementation pending review
>
> Date: 2026-09-03
>
> Product availability: developer-local asset only; not yet routed by Studio
...
本批没有建立 Studio Instrument Catalogue、Channel 10 导入路由或可见乐器选择，因此用户目前仍
不能在 Studio 中选择或自动听到这套鼓组。Velocity 仍只调整单层 Sample 增益；没有 Round Robin、
Velocity Layer、鼓组 Mixer、Room Mic 或新增效果。MI2 才会把已审核 Manifest 接入 Studio，MI3A
才会让导入总谱的 Channel 10 自动选择它，MI5 再进行真实总谱听测和混合 Peak 门禁。
